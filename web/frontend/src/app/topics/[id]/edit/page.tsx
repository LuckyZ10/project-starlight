...
export default function TopicFormPage() {
  ...
  async function loadTopic() {
    try {
      ...
      setSelectedPlatforms(topic.platforms || ["web"]);
    } catch (err) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        showToast("error", isEditing ? "Failed to update topic" : "Failed to create topic");
      }
    } finally {
      setSaving(false);
    }
      ...
      <header className="sticky top-0 z-20 glass border-b border-[var(--border)]">
        <div className="max-w-2xl mx-auto px-4 md:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-base font-semibold">{isEditing ? "Edit Topic" : "New Topic"}</h1>
          </div>
        </div>
      </header>
